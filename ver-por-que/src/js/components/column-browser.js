...
class ColumnBrowser {
                        ...
                        ${columns
                            .map(columnPath => {
                                ...
                                const totalSize = chunks.reduce(
                                    (sum, chunk) => sum + chunk.total_byte_size,
                                    0
                                );
                                const totalPages = chunks.reduce((sum, chunk) => {
                                    let pageCount = 0;
                                    if (chunk.data_pages) {
                                        pageCount += chunk.data_pages.length;
                                    }
                                    if (chunk.dictionary_page) {
                                        pageCount += 1;
                                    }
                                    if (chunk.index_pages) {
                                        pageCount += chunk.index_pages.length;
                                    }
                                ...
                                const schemaColumn = this.app.findColumnInSchema(
                                    this.data.metadata.schema_root,
                                    columnPath
                                );
                                if (schemaColumn) {
    ...
    showPagesView(chunkIndex) {
        ...
        const chunks = this.data.column_chunks.filter(
            chunk => chunk.path_in_schema === this.selectedColumn
        );
